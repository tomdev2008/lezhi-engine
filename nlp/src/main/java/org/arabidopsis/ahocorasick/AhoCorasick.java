...
public class AhoCorasick<E> {
    private State<E> root;
    private boolean prepared;

    public AhoCorasick() {
	this.root = new State<E>(0);
	this.prepared = false;
    }
     ...
     */
    public void add(byte[] keyword, E output) {
	if (this.prepared)
	    throw new IllegalStateException
		("can't add keywords after prepare() is called");
	State<E> lastState = this.root.extendAll(keyword);
     ...
     */
    public Iterator<SearchResult<E>> search(byte[] bytes) {
	return new Searcher<E>(this, this.startSearch(bytes));
    }
    ...
    */
     ...
     */
    SearchResult<E> startSearch(byte[] bytes) {
	if (! this.prepared)
	    throw new IllegalStateException
		("can't start search until prepare()");
	return continueSearch
	    (new SearchResult<E>(this.root, bytes, 0));
    }
     ...
     */
    SearchResult<E> continueSearch(SearchResult<E> lastResult) {
	byte[] bytes = lastResult.bytes;
	State<E> state = lastResult.lastMatchedState;
	for (int i = lastResult.lastIndex; i < bytes.length; i++) {
		...
		return new SearchResult<E>(state, bytes, i+1);
	}
	return null;
    }
