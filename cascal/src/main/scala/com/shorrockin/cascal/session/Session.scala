...
class Session(client: Cassandra.Client, mi: MetaInfo, val defaultConsistency:Consistency) extends SessionTemplate {
  ...
  def verifyInsert[E](col: Column[E]) {
    ...
    if (!mi.keyspaceDescriptors.contains(col.keyspace.value, col.family.value, famType)) {
  ...
  def verifyRemove(container: ColumnContainer[_, _]) {
    if (!mi.keyspaceDescriptors.contains(container.keyspace.value, container.family.value, "Standard") &&
            !mi.keyspaceDescriptors.contains(container.keyspace.value, container.family.value, "Super"))
  ...
  def count[ColumnType, ResultType](containers: Seq[ColumnContainer[ColumnType, ResultType]],
    ...
    if (containers.size > 0) detect {
      ...
      result map {element =>
        (containersByKey(element._1), element._2.intValue)
      }
    } else {
   ...
   */
  def batch(ops: Seq[Operation]): Unit = if (!ops.isEmpty) batch(ops, defaultConsistency)
  ...
  implicit private def convertList[T](v:java.util.List[T]):List[T] = v.asScala.toList
    
  implicit private def convertMap[K,V](m:java.util.Map[K,V]): scala.collection.mutable.Map[K,V] = m.asScala

  implicit private def convertSet[T](s:java.util.Set[T]):scala.collection.mutable.Set[T] = s.asScala
}
