...
class ChannelRecordReader[K, V](split: InputSplit, context: TaskAttemptContext) extends RecordReader[K, V] {
  ...
  override def initialize(split: InputSplit, context: TaskAttemptContext) = {
    val ctx = new TaskAttemptContext(
        extractChannelConfiguration(context, split.asInstanceOf[TaggedInputSplit].channel),
        context.getTaskAttemptID)
    originalRR.initialize(split.asInstanceOf[TaggedInputSplit].inputSplit, ctx)
  }
