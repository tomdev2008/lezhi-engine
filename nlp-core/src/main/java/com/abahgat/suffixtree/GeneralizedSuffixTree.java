...
public class GeneralizedSuffixTree {
    ...
    public List<String> searchMax(String word) {
    	List<String> matches = new ArrayList<String>();
    	int i = 0;
    	while (i < word.length()) {
    		String match = searchPrefix(word, i);
    		if (match.length() > 0) {
    			matches.add(match);
    			i = i + match.length() - 1;
    		}
    ...
    public String searchPrefix(String word, int start) {
    	/*
        ...
        for (int i = start; i < word.length(); ++i) {
            ...
            if (null == currentEdge) {
                ...
                if (!word.regionMatches(i, label, 0, lenToMatch)) {
                	int j = 0;
                	while (j < lenToMatch && label.charAt(j) == word.charAt(i + j)) j++;
                    // the label on the edge does not correspond to the one in the string to search
                	return word.substring(start, i + j);
                }
    ...
    public void put(String key) throws IllegalStateException {
    	put(key, last + 1);
    }
