...
//		System.out.println(splitSnippets("为中国定制 英菲尼迪M35hL北京车展首发"));
	...
	public static List<String> splitSentences(String text) throws IOException {
		...
		for (Term term: Segment.split(text, ToAnalysis.USE_USER_DEFINE)) {
			if (sentenceNatures.contains(term.getNatrue().natureStr) && !isWhiteSpace(prev, term.getName())) {
	...
	public static List<String> splitSnippets(String text) throws IOException {
        ...
        String prev = "";
		for (Term term: Segment.split(text, ToAnalysis.USE_USER_DEFINE)) {
			if ((DictUtil.isUseless(term) || stopwords.contains(TextUtil.stem(term.getName()))) && !isWhiteSpace(prev, term.getName())) {
				if (term.getOffe() > last) {
					String snippet = text.substring(last, term.getOffe()).trim();  
					if (snippet.length() > 0) result.add(snippet);
				}
	...
	private static boolean isWhiteSpace(String prev, String term) {
		return term.length() == 1 && (Character.isWhitespace(term.charAt(0)) || isLetter(prev) && term.charAt(0) == '-');
	}

    private static boolean isLetter(String term) {
        int len = term.length();
        return len > 0 && Character.isLetter(term.charAt(len - 1));
    }
}