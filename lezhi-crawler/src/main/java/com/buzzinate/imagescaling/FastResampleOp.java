package com.buzzinate.imagescaling;
 ...
 * @author Heinz Doerr
 */
public class FastResampleOp extends AdvancedResizeOp {
	...
	public FastResampleOp(ExecutorService threadPool, int destWidth, int destHeight) {
		this(threadPool, DimensionConstrain.createAbsolutionDimension(destWidth, destHeight));
	}

	public FastResampleOp(ExecutorService threadPool, DimensionConstrain dimensionConstrain) {
	...
	public BufferedImage doFilter(BufferedImage srcImg, BufferedImage dest, int dstWidth, int dstHeight) {
        ...
        Future<Integer>[] subtasks = new Future[numberOfThreads-1];
        for (int i=1; i<numberOfThreads;i++) {
        	final int finalI = i;
        	subtasks[i-1] = threadPool.submit(new Callable<Integer>(){
                public Integer call(){
...
//        waitForAllThreads(threads);
	...
	private void waitForAllTasks(Future<Integer>[] futures) {
        try {
            for (Future<Integer> t:futures){
            	t.get(Long.MAX_VALUE, TimeUnit.MILLISECONDS);
            }
        } catch (Exception e) {
            e.printStackTrace();
