	...
	private void analysis(String temp) {
		...
		for (int i = 0; i < length; i++) {
			...
			case 4:
				...
				while (++i < length && (status[temp.charAt(i)] == 4 || status[temp.charAt(i)] == 5 || temp.charAt(i) == '-')) {
					end++;
				}
			...
			case 5:
				...
				int idx = temp.indexOf("..", start) - start;
				if (idx > 0 && idx < end) {
					str = WordAlert.alertNumber(temp, start, idx);
					i = i + idx - end;
				}
				else str = WordAlert.alertNumber(temp, start, end);
				TermNatures natures = TermNatures.NB;
				if (StringUtil.replace(str, ".", "").length() == 0) natures = TermNatures.NULL;
				gp.addTerm(new Term(str, start, natures));
				i--;
				break;
			default:
				start = i;
